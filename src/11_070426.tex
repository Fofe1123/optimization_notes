...
\begin{remark}
    ...
    If $C = \Rn$, then $\Lc{C} = \Rn$, $X$ a polyhedral set (hence retractive), then $A(C \cap X) = AX$ is closed.

\end{remark}

\begin{remark}
    
    $X = \Rn$, $\Rc{X} = \Rn$. $A(X \cap C) = AC$ is closed if $\Rc{C} \cap N(A) = \set{0}$.
...
\begin{proposition}[Book 1.5.4]
    ...
    The closure of the convex hull of a set $C$ is the intersection of all the closed halfspaces containing $C$.
