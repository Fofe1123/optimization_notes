...
\begin{proof}
    ...
    \underline{\textbf{b)}}: assume $f$ is convex and proper, so $\epi{f}$ is nonempty convex set in $\Rnp$, since $f$ is proper, $\epi{f}$ does not contain any vertical line, so there exists $(y,1) \in \Rnp$, $c \in \R$ such that $y^\top x + f(x) \ge c$ for any $x \in \Rn$ (because the epigraph lies above the tangent hyperplane). This implies $f^*(-y) \le -c \implies \sup_x -y^\top x - f(x) \le -c$, so $f^*(y) < +\infty$. Is $f^* \cong -\infty$? No, indeed there exists $\bar{x}$ such that $f(\bar{x})$ is finite ($f$ is proper), hence $f^*(y) = \sup_x y^\top x - f(x) \ge t^\top x - f(\bar{x})$. This shows that $f$ is proper (the same computation holds for the others).
